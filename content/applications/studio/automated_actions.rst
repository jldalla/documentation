...
.. note::
   By default, the scheduler checks for trigger dates every 4 hours, which means that lower
   granularity in time-based automations may not always be honored.
...
destination system (i.e., the system receiving the request), then enter the code to run to determine
the record on which the automation rule should be executed in the :guilabel:`Target Record` field.
...
.. note::
    ...
    - If you wish to use the webhook's content for a purpose other than to find the record, your
      only option is to use an :ref:`studio/automated-actions/action/python-code` action.
...
.. tip::
   ...
   `High` (by starring the task), you can define the trigger :guilabel:`Priority is set to` High`
