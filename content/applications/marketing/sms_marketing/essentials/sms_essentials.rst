...
.. tip::
   ...
   Automated Actions`. Click :guilabel:`Create` or :guilabel:`New` to create a new action.
   ...
   Based on the selection for the :guilabel:`Trigger`, additional fields may populate. Ensure a
   model has been selected before creating any record filters.

   Under :guilabel:`Action To Do`, select :guilabel:`Send SMS Text Message`. Next, set the
   :guilabel:`SMS Template`, and choose whether the SMS message should be logged as a note, by
   ticking the box next to :guilabel:`Log as note`.
   ...
   :guilabel:`Save` the completed automated action to implement the change. Ensure the
   :guilabel:`Active` toggle is set to *on* for the automated action to run.
